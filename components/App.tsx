  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32">
      ...
      <nav className="fixed top-0 w-full glass z-40 hidden md:block border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-10 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <button onClick={handleLogout} className="text-[10px] font-black text-red-400 uppercase tracking-widest">Logout</button>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-10 md:pt-32">
        ...
        {activeTab === 'schedule' && <ScheduleEditor user={user} onUpdate={(u) => { setUser(u); showToast("Schedule synced!"); }} />}
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm px-4">
        <div className="bg-slate-900/95 backdrop-blur-2xl p-3 rounded-[3rem] shadow-2xl border border-white/10 flex justify-between">
          {[
            ...
            <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`p-5 rounded-[2rem] transition-all ${activeTab === t.id ? 'bg-indigo-600 text-white shadow-xl scale-110 -translate-y-2' : 'text-slate-500'}`}>
          ...
          <button onClick={handleLogout} className="md:hidden p-5 text-red-400"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 16l4-4m0 0l-4-4m4 4H7" /></svg></button>
        </div>
      </div>
      <TestBed />
    </div>
